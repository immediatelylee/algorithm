window.onload=function(){
    ...
    input_id.addEventListener('input', function(){
        id_error.style.display = 'none'; // 에러 메시지를 숨깁니다.
    });
    
    input_pw.addEventListener('input', function(){
        pw_error.style.display = 'none'; // 에러 메시지를 숨깁니다.
    });
    ...
    pw_show_hide.addEventListener('click',function(){
        if(i == true){
            ...
            i=false
            // 패스워드 보이기 추가
            document.querySelector('input[name="user_pw"]').setAttribute('type', 'text');
            ...
            i=true
            // 패스워드 숨기기 추가
            document.querySelector('input[name="user_pw"]').setAttribute('type', 'password');
